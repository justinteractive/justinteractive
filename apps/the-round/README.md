...
- `route_stops` — ordered list of dog IDs per mode (`am`/`pm`).
- `route_meta` — last-saved timestamp per mode.
...
| `GET /api/routes/:mode` | `mode` is `am` or `pm` → `{ dogIds, savedAt }` |
| `PUT /api/routes/:mode` | `{ dogIds: [...] }` — replaces the ordered stop list |
